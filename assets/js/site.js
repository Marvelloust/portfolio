(() => {
  ...
  const PROJECT_LINKS = {
    ...
    korago: {
      source: "https://github.com/Marvelloust/Korago",
      demo: "https://marvelloust.github.io/Korago/",
    },
    "relay-copilot": {
      source: "https://github.com/Marvelloust/Relay-Copilot",
      demo: null,
    },
    trace: { source: "https://github.com/Marvelloust/Trace", demo: null },
    ledgerly: { source: "https://github.com/Marvelloust/ledgerly", demo: null },
    fieldnote: {
      source: "https://github.com/Marvelloust/Fieldnote",
      demo: null,
    },
  ...
  if (projectKey && PROJECT_LINKS[projectKey]) {
    ...
    if (hero && (links.source || links.demo)) {
      ...
      if (links.source)
        row.innerHTML += `<a href="${links.source}" target="_blank" rel="noopener">GITHUB REPOSITORY ↗</a>`;
      if (links.demo)
        row.innerHTML += `<a class="is-demo" href="${links.demo}" target="_blank" rel="noopener">LIVE DEMO ↗</a>`;
      hero.after(row);
    }
  }
  ...
  const setWord = () => {
    if (themeWord)
      themeWord.textContent = html.dataset.theme === "dark" ? "LIGHT" : "DARK";
    if (themeMeta)
      themeMeta.setAttribute(
        "content",
        html.dataset.theme === "dark" ? "#0B0E13" : "#F4F0E8",
      );
  };
  setWord();
  themeBtn?.addEventListener("click", (e) => {
    ...
    setTimeout(() => {
      ...
      try {
        localStorage.setItem("mo-build-theme", next);
      } catch (_) {}
      setWord();
  ...
  const updateTop = () =>
    top.classList.toggle("is-visible", scrollY > innerHeight * 0.8);
  ...
  if (matchMedia("(pointer:fine)").matches) {
    ...
    cursor.className = "intent-cursor";
    ...
    document.addEventListener("pointerover", (e) => {
      ...
      const label =
        target?.dataset.cursorIntent ||
        (target?.matches("a")
          ? "OPEN"
          : target?.matches("button")
            ? "ACT"
            : "");
      cursor.dataset.intent = label;
      cursor.querySelector("span").textContent = label;
      cursor.classList.toggle("is-active", !!label);
    });
    document.addEventListener("pointerout", (e) => {
      if (!e.relatedTarget?.closest?.("[data-cursor-intent],a,button"))
