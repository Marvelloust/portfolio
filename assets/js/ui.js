(() => {
  ...
  document.querySelectorAll('[data-stack-system]').forEach(group => {
    ...
    function apply(token) {
      ...
      projects.forEach(project => {
        const tags = (project.dataset.stackProject || '').split(',').map(v => v.trim()).filter(Boolean);
        const match = key === 'all' || tags.includes(key);
        project.hidden = !match;
        project.classList.toggle('is-active', match);
        if (match) visible += 1;
      });
      if (count) count.textContent = `${String(visible).padStart(2,'0')} / ${String(projects.length).padStart(2,'0')} RECORDS`;
      if (read) read.textContent = token?.dataset.readout || 'ALL PERSONAL + TEAM RELATIONSHIPS';
    }

    tokens.forEach(token => token.addEventListener('click', () => apply(token)));
  ...
  const theatreKind = document.querySelector('[data-artifact-theatre]')?.dataset.kind || '';
  ...
  function applyDevicePresentation(){
    ...
    walls.forEach(wall => wall.classList.toggle('artifact-wall--phone', showDevice));
    phoneShots.forEach(shot => shot.classList.toggle('is-phone', showDevice));
    document.querySelector('.artifact-lightbox')?.classList.toggle('is-phone', showDevice);
  }
  ...
  const items = figures.map((figure, index) => {
    ...
    figure.setAttribute('aria-label', `Inspect supplied artifact ${index + 1}`);
