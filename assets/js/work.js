(() => {
  ...
  const MODES={
    all:{code:'ARCHIVE / ALL',title:'Everything currently filed.',copy:'Personal builds lead the cabinet; team contributions remain visibly separated below.'},
    personal:{code:'OWNERSHIP / PERSONAL',title:'Things I can claim as my own build records.',copy:'The lens prioritizes personal builds and removes company contribution records from the active inspection set.'},
    fullstack:{code:'RESPONSIBILITY / FULL-STACK',title:'Surface connected to application rules.',copy:'Records where the work reaches beyond interface presentation into backend, data, integration or system behavior.'},
    mobile:{code:'FORM FACTOR / MOBILE',title:'Touch-first and narrow-screen product work.',copy:'Mobile apps, PWAs and design records where compact hierarchy and touch behavior are part of the build.'},
    design:{code:'EVIDENCE / DESIGN',title:'Interface thinking with explicit design ownership.',copy:'Design records are shown as design evidence rather than presented as shipped engineering.'},
    team:{code:'OWNERSHIP / TEAM',title:'Work completed inside the Neegles team.',copy:'These records stay contribution-only: no independent case-study ownership is implied.'},
    wordpress:{code:'DELIVERY / WORDPRESS',title:'WordPress delivery completed with Neegles.',copy:'CMS and WordPress contributions are grouped as team delivery rather than personal product builds.'}
  };

  const modesFor=el=>(el.dataset.mode||'').split(/\s+/).filter(Boolean);
  function applyMode(mode){
    buttons.forEach(b=>b.classList.toggle('is-active',b.dataset.workFilter===mode));
    document.body.classList.remove('mode-all','mode-personal','mode-fullstack','mode-mobile','mode-design','mode-team','mode-wordpress');
    document.body.classList.add(`mode-${mode}`);

    let personalCount=0, teamCount=0;
    items.forEach(item=>{
      const modes=modesFor(item);
      const show=mode==='all' || (mode==='personal' ? modes.includes('personal') : modes.includes(mode));
    ...
    if(teamSection){
      const showTeam=mode==='all'||mode==='team'||mode==='wordpress';
    ...
    if(status){
      const ownership=mode==='team'||mode==='wordpress'?'TEAM CONTRIBUTION':mode==='all'?'ARCHIVE':'PERSONAL BUILD';
    ...
    if(lens){
      ...
      lens.style.setProperty('--lens-progress',`${Math.max(16,Math.min(100,total/Math.max(1,items.length+teamItems.length)*100))}%`);
  ...
  buttons.forEach(btn=>btn.addEventListener('click',()=>applyMode(btn.dataset.workFilter||'all')));
  applyMode('all');
  ...
  if(matchMedia('(pointer:fine)').matches){
    ...
    preview.innerHTML='<div class="inspection-preview__label"><span class="mono"></span><b></b></div><div class="inspection-preview__image"><img alt=""></div><div class="inspection-preview__scan"></div>';
    document.body.append(preview);
    let tx=innerWidth*.68,ty=innerHeight*.46,cx=tx,cy=ty,active=null;
    const img=preview.querySelector('img'), id=preview.querySelector('.mono'), title=preview.querySelector('b');
    function activate(item){
      ...
      preview.style.setProperty('--preview-accent',item.dataset.accent||'#7C6CF2');
    ...
    function clear(){
      ...
      page.classList.remove('is-inspecting');preview.classList.remove('is-visible');
    }
    items.forEach(item=>{
      item.addEventListener('pointerenter',()=>activate(item));
      item.addEventListener('focusin',()=>activate(item));
      item.addEventListener('pointermove',e=>{tx=e.clientX+28;ty=e.clientY-120;});
      item.addEventListener('pointerleave',clear);
      item.addEventListener('focusout',e=>{if(!item.contains(e.relatedTarget))clear()});
    });
    const loop=()=>{
      cx+=(tx-cx)*.14;cy+=(ty-cy)*.14;
      const w=preview.offsetWidth||420,h=preview.offsetHeight||300;
  ...
  function openFlagship(link,item){
    ...
    if(!href)return;
    if(reduce.matches || innerWidth<901){ location.href=href; return; }
    ...
    overlay.style.setProperty('--retrieval-accent',item.dataset.accent||'#7C6CF2');
    ...
    requestAnimationFrame(()=>requestAnimationFrame(()=>overlay.classList.add('is-expanded')));
